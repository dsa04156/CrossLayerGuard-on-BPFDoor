...
#define BPF_OBJ_DIR "/usr/local/lib/crosslayer-ebpf"
...
static uint32_t my_ip_h = 0;   // 호스트 바이트 오더
...
static bool is_internal_or_loopback(uint32_t ip_h) {
    // 루프백
    if ((ip_h >> 24) == 127) return true;
    // 10.0.0.0/8
    if ((ip_h & 0xFF000000) == 0x0A000000) return true;
    // 172.16.0.0/12
    if ((ip_h & 0xFFF00000) == 0xAC100000) return true;
    // 192.168.0.0/16
    if ((ip_h & 0xFFFF0000) == 0xC0A80000) return true;
    return false;
}

void get_my_ip(const char *ifname) {
    struct ifaddrs *ifap, *ifa;
    if (getifaddrs(&ifap) != 0) return;
    for (ifa = ifap; ifa; ifa = ifa->ifa_next) {
        if (ifa->ifa_addr && ifa->ifa_addr->sa_family == AF_INET
         && strcmp(ifa->ifa_name, ifname) == 0) {
            struct sockaddr_in *s = (void*)ifa->ifa_addr;
            my_ip_h = ntohl(s->sin_addr.s_addr);
            break;
        }
    }
    freeifaddrs(ifap);
...
static void handle_xdp(void *ctx, int cpu, void *data, __u32 sz) {
    ...
    uint32_t dst_h = ntohl(m->dst_ip);
    uint32_t src_h = ntohl(m->src_ip);
    if (is_internal_or_loopback(src_h) && is_internal_or_loopback(dst_h))
        return;
...
int main(int argc, char **argv) { 
    ...
    // }
    if (argc != 3) {
        fprintf(stderr, "Usage: %s <bpf_obj_dir> <interface>\n", argv[0]);
        return 1;
    }
    ...
    obj_ctrl = bpf_object__open_file(BPF_OBJ_DIR "/ctrl_probe.o", NULL);
    if (libbpf_get_error(obj_ctrl)) {
        fprintf(stderr, "ERROR: opening ctrl_probe.o: %s\n", strerror(-libbpf_get_error(obj_ctrl)));
        return EXIT_FAILURE; 
    }
    if ((err = bpf_object__load(obj_ctrl))) {
        fprintf(stderr, "ERROR: loading ctrl_probe.o: %d (%s)\n", err, strerror(-err));
   ...
   if (libbpf_get_error(pb_ctrl) || !pb_ctrl) {
    ...
    printf("My IP: %u.%u.%u.%u\n",
       (my_ip_h>>24)&0xFF,(my_ip_h>>16)&0xFF,
       (my_ip_h>>8)&0xFF,(my_ip_h>>0)&0xFF);
    printf("Using interface '%s' (ifindex %d)\n", ifname, ifindex);

    obj_xdp = bpf_object__open_file(BPF_OBJ_DIR "/xdp_record.o", NULL);
    if (libbpf_get_error(obj_xdp)) { fprintf(stderr, "ERROR: opening xdp_record.o: %s\n", strerror(-libbpf_get_error(obj_xdp))); goto app_cleanup; }
    if ((err = bpf_object__load(obj_xdp))) { fprintf(stderr, "ERROR: loading xdp_record.o: %d (%s)\n", err, strerror(-err)); goto app_cleanup; }
    
    prog = bpf_object__find_program_by_name(obj_xdp, "xdp_record"); 
    if (!prog) { fprintf(stderr, "ERROR: finding program 'xdp_record' in xdp_record.o\n"); goto app_cleanup; }
    ...
    if (map_fd_xdp < 0) { fprintf(stderr, "ERROR: finding 'events' map in xdp_record.o (err %d)\n", map_fd_xdp); goto app_cleanup; }
    ...
    obj_tc = bpf_object__open_file(BPF_OBJ_DIR "/tc_record.o", NULL);
    if (libbpf_get_error(obj_tc)) { fprintf(stderr, "ERROR: opening tc_record.o: %s\n", strerror(-libbpf_get_error(obj_tc))); goto app_cleanup; }
    if ((err = bpf_object__load(obj_tc))) { fprintf(stderr, "ERROR: loading tc_record.o: %d (%s)\n", err, strerror(-err)); goto app_cleanup; }
    
    prog = bpf_object__find_program_by_name(obj_tc, "tc_record"); 
    if (!prog) { fprintf(stderr, "ERROR: finding program 'tc_record' in tc_record.o\n"); goto app_cleanup; }
    ...
    if (map_fd_tc < 0) { fprintf(stderr, "ERROR: finding 'events' map in tc_record.o (err %d)\n", map_fd_tc); goto app_cleanup; }
    ...
    obj_sock = bpf_object__open_file(BPF_OBJ_DIR "/sock_record.o", NULL);
    if (libbpf_get_error(obj_sock)) { fprintf(stderr, "ERROR: opening sock_record.o: %s\n", strerror(-libbpf_get_error(obj_sock))); goto app_cleanup; }
    if ((err = bpf_object__load(obj_sock))) { fprintf(stderr, "ERROR: loading sock_record.o: %d (%s)\n", err, strerror(-err)); goto app_cleanup; }
    
    prog = bpf_object__find_program_by_name(obj_sock, "sock_record"); 
    if (!prog) { fprintf(stderr, "ERROR: finding 'sock_record' in sock_record.o\n"); goto app_cleanup; }
    ...
    if (map_fd_sock < 0) { fprintf(stderr, "ERROR: finding 'events' map in sock_record.o (err %d)\n", map_fd_sock); goto app_cleanup; }
